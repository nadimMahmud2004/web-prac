let books = [
  {
    ...
    quantity: 5,
    available: 3,
    cover: "https://placehold.co/150x220/007bff/white?text=Gatsby",
  ...
  },
  {
    ...
    quantity: 3,
    available: 1,
    cover: "https://placehold.co/150x220/28a745/white?text=Mockingbird",
  ...
  },
  {
    ...
    quantity: 7,
    available: 7,
    cover: "https://placehold.co/150x220/ffc107/black?text=1984",
  ...
  },
  {
    ...
    quantity: 4,
    available: 2,
    cover: "https://placehold.co/150x220/dc3545/white?text=Pride+Prejudice",
  ...
  },
  {
    ...
    quantity: 6,
    available: 6,
    cover: "https://placehold.co/150x220/17a2b8/white?text=Hobbit",
...
function updateStudentBorrowedCounts() {
  students.forEach((student) => {
    student.borrowedBooksCount = borrowedItems.filter(
      (item) => item.studentId === student.id && !item.returned,
    ).length;
  });
}
...
function renderDashboard() {
  ...
  const currentlyBorrowed = borrowedItems.filter(
    (item) => !item.returned,
  ).length;
  const totalStudents = students.length;
  const today = new Date().toISOString().split("T")[0];
  const overdueBooksCount = borrowedItems.filter(
    (item) => !item.returned && item.dueDate < today,
  ).length;
...
function renderQuickStatsChart() {
  /* Same as before, ensure it's called */
  ...
  new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        title: { display: true, text: "Number of Book Titles by Genre" },
      },
    },
  });
}
...
function renderBookCatalog() {
  /* Mostly same, ensure DataTables re-init */
  let tableRows = books
    .map(
      (book) => `
  ...
  document
    .querySelectorAll(".borrow-book-btn")
    .forEach((btn) =>
      btn.addEventListener("click", (e) =>
        navigateTo("borrowBook", {
          bookId: parseInt(e.currentTarget.dataset.bookId),
        }),
      ),
    );
  document
    .querySelectorAll(".view-book-btn")
    .forEach((btn) =>
      btn.addEventListener("click", (e) =>
  ...
  document
    .querySelectorAll(".edit-book-btn")
    .forEach((btn) =>
      btn.addEventListener("click", (e) =>
        navigateTo("addBook", {
          bookId: parseInt(e.currentTarget.dataset.bookId),
...
function deleteBook(bookId) {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      const isBorrowed = borrowedItems.some(
...
function renderAddNewBookForm(params = {}) {
  /* Mostly same, ensure flatpickr re-init */
  ...
  if (!isEdit) {
    ...
    // For edit mode, ensure available is not more than quantity
    document.getElementById("bookQuantity").addEventListener("input", (e) => {
      const newQuantity = parseInt(e.target.value) || 0;
      const currentBorrowed = bookToEdit.quantity - bookToEdit.available;
      document.getElementById("bookAvailable").value = Math.max(
        0,
        newQuantity - currentBorrowed,
      );
    });
  }
  ...
  document
    ...
    .addEventListener("submit", function (event) {
      ...
      const cover =
        ...
        `https://placehold.co/150x220/6c757d/white?text=${title.substring(0, 10).replace(/\s+/g, "+")}`;
      ...
      if (isEdit && bookId) {
        ...
        if (bookIndex > -1) {
          ...
          if (newAvailable < 0) {
            // This case should ideally be prevented by disabling quantity reduction below borrowed count
            showMessage(
              "Error",
              `Cannot reduce quantity below the number of currently borrowed copies (${currentlyBorrowed}).`,
              "error",
            );
            return;
          }
...
function renderBorrowBookForm(params = {}) {
  ...
  const availableBooksOptions = books
    ...
    .map(
      (b) =>
        `<option value="${b.id}" ${preselectedBookId && b.id === preselectedBookId ? "selected" : ""}>${b.title} (${b.available} avail.)</option>`,
    )
    .join("");
  ...
  document
    ...
    .addEventListener("submit", function (event) {
      ...
      if (book && student && book.available > 0) {
        ...
        showMessage(
          "Error",
          "Borrow request failed. Book unavailable or student not found.",
          "error",
        );
      }
    });
...
// Return Book
function renderReturnBookPage() {
  /* Mostly same, ensure DataTables re-init */
  let itemsToReturnHtml = borrowedItems
    ...
    .join("");
  if (!borrowedItems.filter((item) => !item.returned).length) {
    itemsToReturnHtml = `<tr><td colspan="7" class="text-center py-3">No books currently pending return.</td></tr>`;
  }
  mainContent.innerHTML = `
                <div class="content-section" data-aos="fade-up">
  ...
  if ($.fn.DataTable.isDataTable("#returnBookTable")) {
    $("#returnBookTable").DataTable().destroy();
  }
  $("#returnBookTable").DataTable({
    responsive: true,
    pageLength: 10,
    order: [[3, "asc"]],
    columnDefs: [{ targets: [6], orderable: false, searchable: false }],
  });
  document.querySelectorAll(".return-now-btn").forEach((button) => {
    /* Event listener logic same as before */
    button.addEventListener("click", (e) => {
      ...
      if (!borrowedItem) {
        showMessage("Error", "Borrowed item not found.", "error");
        return;
      }
      let confText = "Mark this book as returned?";
      if (lateFee > 0) {
        confText += ` Late fee of $${lateFee.toFixed(2)} applies.`;
      }
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          borrowedItem.returnDate = new Date().toISOString().split("T")[0];
          ...
          showMessage(
            "Book Returned!",
            `Book marked as returned. ${lateFee > 0 ? "Late fee processed." : ""}`,
            "success",
          );
...
function renderStudentRecords() {
  ...
  let tableRows = students
    ...
    .join("");
  if (!students.length) {
    tableRows = `<tr><td colspan="6" class="text-center py-3">No students registered yet. Add students in 'Manage Students'.</td></tr>`;
  }
  mainContent.innerHTML = `
                <div class="content-section" data-aos="fade-up">
  ...
  document
    .querySelectorAll(".view-student-history-btn")
    .forEach((btn) =>
      btn.addEventListener("click", (e) =>
        viewStudentBorrowHistory(parseInt(e.currentTarget.dataset.studentId)),
      ),
    );
}

function viewStudentBorrowHistory(studentId) {
  /* Same as before */
  const student = students.find((s) => s.id === studentId);
  if (!student) {
  ...
  let historyHtml =
    '<p class="text-muted">No borrow history for this student.</p>';
  if (history.length > 0) {
    historyHtml = `<ul class="list-group list-group-flush">${history
      .map((item) => {
        const book = books.find((b) => b.id === item.bookId);
  ...
  Swal.fire({
    title: `<i class="fas fa-user-graduate me-2"></i> History for ${student.name}`,
    html: `<div class="text-start">${historyHtml}</div>`,
    icon: "info",
    confirmButtonText: "Close",
    width: "600px",
  });
...
function renderManageStudentsPage() {
  ...
  let tableRows = students
    ...
    .join("");
  if (!students.length) {
    tableRows = `<tr><td colspan="6" class="text-center py-3">No students registered yet.</td></tr>`;
  }
  mainContent.innerHTML = `
                <div class="content-section" data-aos="fade-up">
  ...
  if ($.fn.DataTable.isDataTable("#manageStudentsTable")) {
    $("#manageStudentsTable").DataTable().destroy();
  }
  ...
  document
    .getElementById("addNewStudentBtn")
    .addEventListener("click", () => renderAddEditStudentForm());
  document
    .querySelectorAll(".edit-student-btn")
    .forEach((btn) =>
      btn.addEventListener("click", (e) =>
        renderAddEditStudentForm({
          studentId: parseInt(e.currentTarget.dataset.studentId),
          isEdit: true,
        }),
      ),
    );
  document
    ...
    .forEach((btn) =>
      btn.addEventListener("click", (e) =>
        deleteStudent(parseInt(e.currentTarget.dataset.studentId)),
      ),
    );
}
...
function renderAddEditStudentForm(params = {}) {
  ...
  if (isEdit && studentIdToEdit) {
    ...
    if (!studentToEdit) {
      showMessage("Error", "Student not found for editing.", "error");
  ...
  document
    .getElementById("addEditStudentForm")
    .addEventListener("submit", function (event) {
      event.preventDefault();
      event.stopPropagation();
      const form = event.target;
      ...
      if (isEdit && studentId) {
        ...
        if (studentIndex > -1) {
          ...
          showMessage("Error", "Could not find student to update.", "error");
        }
      } else {
        const newStudent = {
...
function deleteStudent(studentId) {
  ...
  if (
    borrowedItems.some((item) => item.studentId === studentId && !item.returned)
  ) {
    showMessage(
      "Cannot Delete",
      `${student.name} has active borrowed books. Return books first.`,
      "error",
    );
    return;
  }
...
function renderIssuedBooks() {
  ...
  let tableRows = borrowedItems
    ...
    .join("");
  if (!borrowedItems.filter((item) => !item.returned).length) {
    tableRows = `<tr><td colspan="6" class="text-center py-3">No books are currently issued.</td></tr>`;
  }
  mainContent.innerHTML = `
                <div class="content-section" data-aos="fade-up">
  ...
  if ($.fn.DataTable.isDataTable("#issuedBooksTable")) {
    $("#issuedBooksTable").DataTable().destroy();
  }
  ...
  document.querySelectorAll(".mark-returned-quick-btn").forEach((button) => {
    /* Event listener logic same as before */
    button.addEventListener("click", (e) => {
      const borrowId = parseInt(e.currentTarget.dataset.borrowId);
      const borrowedItem = borrowedItems.find((item) => item.id === borrowId);
      ...
      Swal.fire({
        title: "Confirm Return",
        text: `Mark "${books.find((b) => b.id === borrowedItem.bookId)?.title}" as returned?`,
        icon: "question",
        showCancelButton: true,
        confirmButtonColor: "#28a745",
        confirmButtonText: "Yes, return!",
...
function renderLateReturns() {
  ...
  const today = new Date().toISOString().split("T")[0];
  let tableRows = borrowedItems
    .filter((item) => !item.returned && item.dueDate < today)
    .map((item) => {
      const book = books.find((b) => b.id === item.bookId);
      const student = students.find((s) => s.id === item.studentId);
      if (!book || !student) return "";
      const diffDays = Math.max(
        0,
        Math.ceil(
          (new Date(today) - new Date(item.dueDate)) / (1000 * 60 * 60 * 24),
        ),
      );
      const lateFee = (diffDays * 1.0).toFixed(2);
    ...
    .join("");
  if (
    !borrowedItems.filter((item) => !item.returned && item.dueDate < today)
      .length
  ) {
  ...
  if ($.fn.DataTable.isDataTable("#lateReturnsTable")) {
    $("#lateReturnsTable").DataTable().destroy();
  }
  $("#lateReturnsTable").DataTable({
    responsive: true,
    pageLength: 10,
    order: [[3, "desc"]],
    columnDefs: [{ targets: [5], orderable: false, searchable: false }],
  });
  document.querySelectorAll(".remind-student-btn").forEach((button) => {
    /* Event listener logic same as before */
    button.addEventListener("click", (e) => {
      ...
      showMessage(
        ...
        `Reminder sent to ${student.name} for "${bookTitle}".`,
        "info",
      );
    });
  });
  document.querySelectorAll(".mark-returned-late-btn").forEach((button) => {
    /* Event listener logic same as before */
    button.addEventListener("click", (e) => {
      const borrowId = parseInt(e.currentTarget.dataset.borrowId);
      const lateFee = e.currentTarget.dataset.lateFee;
      const borrowedItem = borrowedItems.find((item) => item.id === borrowId);
      if (!borrowedItem) return;
      Swal.fire({
        title: "Confirm Return & Fine",
        text: `Mark "${books.find((b) => b.id === borrowedItem.bookId)?.title}" returned? Fine: $${lateFee}.`,
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#28a745",
        ...
        if (result.isConfirmed) {
          borrowedItem.returned = true;
          borrowedItem.returnDate = new Date().toISOString().split("T")[0];
          borrowedItem.finePaid = true; // Simulate fine paid
          const book = books.find((b) => b.id === borrowedItem.bookId);
          if (book) book.available++;
          updateStudentBorrowedCounts();
          showMessage(
            "Book Returned & Fine Processed!",
            `Fine of $${lateFee} processed.`,
            "success",
          );
          renderLateReturns();
        }
...
function renderReports() {
  /* Mostly same, ensure charts re-init */
  mainContent.innerHTML = `
                <div class="content-section" data-aos="fade-up">
...
function renderGenrePieChart() {
  ...
  new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        legend: { position: "top" },
        title: { display: true, text: "Book Titles by Genre" },
      },
    },
  });
}
function renderMonthlyBorrowsChart() {
  /* Same as before */
  ...
  new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        title: { display: true, text: "Monthly Borrowing Trend (Sample)" },
      },
    },
  });
}
function renderAvailabilityChart() {
  ...
  new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        legend: { position: "top" },
        title: { display: true, text: "Overall Book Availability" },
      },
    },
  });
}
  ...
  const faqs = [
    {
      ...
      a: "Navigate to the 'Book Catalog' section. Use the search bar to find books by title, author, ISBN, or genre. Click column headers to sort.",
    },
    {
      q: "How do I borrow a book?",
      a: "Go to 'Borrow Book', select student and book, set return date, and confirm. Or, use the 'Borrow' button in the 'Book Catalog'.",
    },
    {
      q: "What is the late return policy?",
      a: "A late fee (e.g., $1.00/day) applies for overdue books. Check 'Late Returns' for details.",
    },
    {
      q: "How to view my borrow history?",
      a: "Librarians can view student history in 'Student Records' or 'Manage Students'. A student portal is planned.",
    },
    {
      q: "Adding new books (admin)?",
      a: "Use 'Add New Book', fill details, and save. Ensure ISBN and quantity are correct.",
    },
    {
      q: "Damaged or lost books?",
      a: "Report to library staff immediately. A replacement fee may apply as per policy.",
    },
  ];
  const faqHtml = faqs
    .map(
...
const navLinks = document.querySelectorAll(
  "#sidebar .nav-link, .footer a[data-page]",
);
...
function navigateTo(page, params = {}) {
  mainContent.innerHTML =
    '<div class="d-flex justify-content-center align-items-center" style="min-height: 300px;"><div class="spinner-border text-primary" role="status" style="width: 3rem; height: 3rem;"><span class="visually-hidden">Loading...</span></div></div>';
  ...
  setTimeout(() => {
    switch (page) {
      ...
      case "catalog":
        ...
        break;
      case "addBook":
        renderAddNewBookForm(params);
        break;
      case "borrowBook":
        renderBorrowBookForm(params);
        break;
      case "returnBook":
        renderReturnBookPage();
        break;
...
document
  .querySelector(".sidebar-header a.navbar-brand")
